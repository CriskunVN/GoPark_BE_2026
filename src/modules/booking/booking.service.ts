import { BadRequestException, Body, Injectable, NotFoundException, Param, ValidationPipe } from '@nestjs/common';
...
export class BookingService {
    ...
    getAllBooking(){
        return this.bookingRepository.find({
            ...
            relations : ['user','user.profile','vehicle','slot','parkingLot','invoice'] //tên các trường trong database-ko phải tên database
        });
    }
    ...
    async updateBooking(id:number,bookingdto) {
        ...
        if (bookingdto.vehicle_id) updateData.vehicle = { id: bookingdto.vehicle_id };
        if (bookingdto.parking_lot_id) updateData.parkingLot = { id: bookingdto.parking_lot_id };
